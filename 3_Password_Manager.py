...
class GUIReturningUserWindow:
    ...
    def view_passwords_frame(self):
        ...
        # Display user's saved passwords from file
        find_dividers = False
        with open('Saved_Passwords.txt', 'r') as saved_passwords:
            count = len(saved_passwords.readlines())
            saved_passwords.seek(0)
            temp_data = []

            # Identify user's passwords with dividers placed within file
            for i in saved_passwords:
                i = i.strip()
    ...
    def remove_password(self):
        ...
        with open('Saved_Passwords.txt', 'r+') as passwords:
            read_passwords = passwords.readlines()
            passwords.seek(0)
            
            search_password = False
            # Get line number of password
            for i, j in enumerate(read_passwords):
                if j.strip() == 'Password: ' + remove_message:
                    search_password = True
                    break
                
            # Rewrite passwords to file without the user's entered password
            if search_password:
                passwords.seek(0)
                for k, t in enumerate(read_passwords):
                    if k != i and k != i - 1 and k != i - 2 and k != i + 1:
                        passwords.write(t)

                # Ensure no leftover lines remain when rewriting
                passwords.truncate()
